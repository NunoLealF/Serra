...
#ifndef SERRA_KERNEL_CONSTRUCTORS_GRAPHICS_H
#define SERRA_KERNEL_CONSTRUCTORS_GRAPHICS_H
  ...
  typedef enum _consoleType : uint8 {
    ...
    UnknownConsoleType = 0, // (Unknown or unsupported console)

    VgaConsoleType = 1, // (Uses framebuffer, 16-bit characters with ((Color << 8) | Char))
    EfiConsoleType = 2, // (Uses gST->ConOut, or efiSimpleTextOutputProtocol)

    GraphicalConsoleType = 3 // (Uses graphics mode, check for bitmap font.)
